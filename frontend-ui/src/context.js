...
const Context = ({ children }) => {
    ...
    if (userdata != null) {
      const userData = {
        ...
        lastname: userdata.name
          .substring(hasWhiteSpaceforLast(userdata.name), userdata.name.length)
          .trim(),
  ...
  useEffect(() => {
    ...
    if (selectedColor.length > 0)
      filteredProducts = tempallpro.filter((c) =>
        selectedColor.includes(c.color.toLowerCase())
      );
      ...
      selectedCategory.filter((s) => {
        filter2.push(
          ...filteredProducts.filter(
            (c) => c.category.toLowerCase() == s.toLowerCase()
          )
        );
      });
    ...
    if (
      ...
      // Modification and addition by Om Shrivastava on 14-07-2024
      // Reason : Remove the 8 product condition
      ...
      setReload(!reload);
      finalFilter = filteredProducts;
      // End of modification and addition by Om Shrivastava on 14-07-2024
      // Reason : Remove the 8 product condition
    } else finalFilter = tempallpro;
  ...
  const setcheck = (e) => {
    if (e.target.value == 1) {
      ...
      setCategoryProduct([
        ...CategoryProduct.sort(function (a, b) {
