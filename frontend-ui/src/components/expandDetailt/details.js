...
const Details = (props) => {
 ...
 async function gettingDetail(){
    await DetailApi(id).then(r=>{setDetails({...r})})
  }
        ...
        <div className={styles['container02']}>
          ...
          <div className={styles['container03']}>
            ...
            <div class={sizeCond?sty:"rating-container face"}>
              {pushData?<Shake>   <div class="rating" > 
                      <form class="rating-form">

                      <label for={details.id}>
                          <input type="radio" name={details.id} class="super-happy" id={details.id} value="Short" onChange={e=>onChange(e.target.value)}/>
                          <span class="span">S</span>
                      </label>

                        <label for={details.id*44}>
                          <input type="radio" name={details.id} class="happy" id={details.id*44} value="Medium" onChange={e=>onChange(e.target.value)} />
                      ...
                      <label for={details.id*88}>
                          <input type="radio" name={details.id} class="sad" id={details.id*88} value="Large" onChange={e=>onChange(e.target.value)}/>
                          <span class="span">L</span>
                      </label>

                      <label for={details.id*108}>
                          <input type="radio" name={details.id} class="super-sad" id={details.id*108} value="Extra Large" onChange={e=>onChange(e.target.value)}/>
                          <span class="span">XL</span>
                      </label>

                      <label for={details.id*126}>
                          <input type="radio" name={details.id} class="super-sad" id={details.id*126} value="Extra Extra Large" onChange={e=>onChange(e.target.value)}/>
                          <span class="span">XXL</span>
                      </label>
                    
                      </form>
                  </div>
                  <div class="rating" style={{color:"red",fontSize:"0.8rem",marginLeft:'10px'}}>Please select one size</div>
                  </Shake>:
                  <>
                  <div class="rating" > 
                      <form class="rating-form">
                 ...
                 {notAvai?<div id="rating" style={{color:"red",fontSize:"0.8rem",marginLeft:'10px',display:"block"}}>This size is not available.</div>:null}
                ...
                <span className={styles['text02']}>Size</span>
              </div>
             ...
             {check()?<button
              ...
              {like.filter(l=>l.id===details.id).length>0?<AiFillHeart style={{color:"red",marginTop:"20px",width:"25px",height:"25px"}} onClick={e=>LikedSave(details)}/> :<AiOutlineHeart style={{marginTop:"20px",width:"25px",height:"25px"}} onClick={e=>LikedSave(details)}/>}
                 <a href='https://wa.me/916264170187'> <AiOutlineWhatsApp style={{marginTop:"20px",width:"25px",height:"25px",marginLeft:"10px"}}/></a> 
              </div>
              </div>
              <h1 className={styles['text03']} >ABOUT THE PRODUCT</h1>
              <span className={styles['text04']}>{details.description}</span>
              ...
              <h1 className={styles['text03']} onClick={check}>PRODUCT DETAILS</h1>
              <div className={styles['container06']}>
                <span className={styles['text05']}>Style Code </span>
                <span className={styles['text06']}>:</span>
                <span className={styles['text07']}>{details.style_code}</span>
              </div>
              <div className={styles['container07']}>
              ...
              <span className={styles['text26']}>Submit your customisation details On <a href="https://wa.me/916264170187" style={{fontSize:"1rem"}}>Whatsapp</a></span>
     ...
     <span className={styles['text28']}>Text</span> 
      ...
      <div className={styles.gal}>
            <div className={styles.image_gallery}>
                ...
                <InnerImageZoom className={styles.img}
                  src={config.apiBaseURL+details.img_sub1} zoomSrc={config.apiBaseURL+details.img_sub1} />
                </div>
                </div>
                <div className={styles.column}>
                <div className={styles.image_item}>
                <InnerImageZoom className={styles.img}
                  src={config.apiBaseURL+details.img_sub2} zoomSrc={config.apiBaseURL+details.img_sub2} />
                ...
                <div className={styles.image_item}>
                <InnerImageZoom className={styles.img}
                  src={config.apiBaseURL+details.img_sub3} zoomSrc={config.apiBaseURL+details.img_sub3} />
                </div>
                </div>
        ...
        <div style={{width:"80vw",height:"70vh",marginTop:"6%",zIndex:"0"}}>
        <div style={{ fontSize: "24px",lineHeight:"32px",letterSpacing:"3px",marginBottom:"20px",fontFamily:"Rawson-Regular",marginLeft:"-10px",fontWeight:"600"}}>RECENTLY VIEWED PRODUCTS</div>
           <Slider/>
        </div>
